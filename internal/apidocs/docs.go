...
const docTemplate = `{
    ...
    "paths": {
        ...
        "/api/v1/platform/mcp-workbench": {
            "get": {
                "description": "Read-only MCP scan workbench projection. It uses one fixed 30 UTC-day window [utcDay(now)-29d, utcDay(now)+1d). Users receive only their own tasks and reports; auditors and administrators receive global scope. running counts dispatching/running MCP tasks and pending counts pending/dispatch_unknown MCP tasks by created_at. completed_30d counts MCP reports and high_risk sums their safe high-risk counts by completed_at. active_tasks contains at most 10 nonterminal tasks ordered updated_at DESC then task ID DESC. recent_risks contains at most 5 safe highlights ordered high, medium, low, then completed_at DESC and report ID DESC. Missing or unrecognized legacy source values are legacy_unknown. Legacy reports lacking trusted category/severity use generic other highlights derived only from risk counts. The response is a server-owned whitelist and never returns raw results, scanner findings, request content, repository or service endpoints, model IDs, authorization data, headers, attachments, parameters, or logs.",
                "produces": [
                    "application/json"
                ],
                "responses": {
                    "200": {
                        "description": "Authorized safe MCP workbench projection.",
                        "schema": {
                            "$ref": "#/definitions/mcpworkbench.View"
                        }
                    },
                    "401": {
                        "description": "Unauthenticated."
                    ...
                    "500": {
                        "description": "MCP workbench aggregation failed; internal details are not exposed."
                    }
                },
                "summary": "Get read-only MCP scan workbench",
                "tags": [
                    "platform-mcp-workbench"
                ]
            }
        },
        "/api/v1/public/brand": {
        ...
        "/api/v1/platform/reports/{reportID}": {
            "get": {
                "description": "Returns the safe immutable RenderModel projection for one report. Users read only their own reports; auditors and administrators have global read access. raw engine results, stored render JSON, Logo bytes, owner IDs, paths, and mutable brand records are never exposed. MCP technical findings are generic summaries without targets, repository identifiers, or scanner free text.",
                "parameters": [
                    {
                        "in": "path",
                        "name": "reportID",
        ...
        "/api/v1/platform/models/{modelID}": {
            ...
            "put": {
                "description": "Updates an authorized private or global model and audits the change. Empty or masked Token preserves the existing encrypted Token only when the normalized full base URL is unchanged. Changing the base URL, including its API path, requires a new Token. Equivalent trailing slash and default-port normalization do not count as a change.",
                "parameters": [
                    {
                        "in": "path",
                        "name": "modelID",
        ...
        "/api/v1/platform/tasks": {
            "get": {
                ...
                "responses": {
                    ...
                    "409": {
                        "description": "MCP_SPECIALIZED_ENDPOINT_REQUIRED. specialized_path identifies the dedicated MCP endpoint."
                    },
                    "500": {
                    ...
                    },
                    {
                        ...
                        "schema": {
                            "properties": {
                                ...
                                "params": {
                                    "type": "object",
                                    "description": "Exact per-task schema described by this operation; agent_scan accepts only agent_id and eval_model_id, both required governed string references. skills_scan accepts only the required model_id string. Unknown, arbitrary, and credential-bearing fields are rejected. ai_infra_scan additionally accepts optional paired target_credential_id (opaque string) and target_credential_revision (positive int64), never target_auth or raw headers. Selected credentials restrict content to explicit same-origin HTTP or HTTPS URLs (HTTP and HTTPS are accepted by default) and prohibit attachments."
                                },
                                "remark": {
        ...
        "/api/v1/platform/mcp-scans": {
            "get": {
                "summary": "List MCP scans",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Only MCP canonical and historical aliases. No engine polling or raw parameter projection. Source kind is repository, service or legacy_unknown.",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "responses": {
                    "200": {
                        ...
                        "schema": {
                            "$ref": "#/definitions/mcpscans.ListResponse"
                        }
                    },
                    "400": {
                        "description": "Invalid strict request or server-owned outbound policy rejected."
                    },
                    "401": {
                        "description": "Authentication required."
                    },
                    "403": {
                        "description": "Role, first-password-change or CSRF gate denied."
                    },
                    "404": {
                        "description": "Absent or inaccessible opaque resource."
                    },
                    "409": {
                        "description": "IDEMPOTENCY_KEY_REUSED, MCP_CONNECTION_VERSION_CONFLICT or state conflict; refresh before a new operation."
                    },
                    "500": {
                        "description": "Fixed safe error only; no upstream diagnostics or secrets."
                    },
                    "503": {
                        "description": "Required governed runtime or egress unavailable."
                    }
                }
            },
            "post": {
                "summary": "Create MCP scan",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Dedicated mcp_scan only. Flat strict JSON object, exact case-sensitive keys; null, duplicates and unknown fields rejected. source_kind repository accepts exactly one HTTPS Git repository_url or 1..10 ready owner-scoped MCP attachment_ids; both service fields and authorization_confirmed must be absent. service requires connection_config_id, exact immutable connection_config_version, authorization_confirmed=true and no repository fields. Only enabled, tested and policy-approved saved connections are eligible. model_id is an optional governed reference; thread defaults to 4 and is 1..32. Language is server-fixed zh_CN; task_type, language, country_iso_code, content, headers and secrets are forbidden. Source-related audit metadata contains source_kind and the Boolean authorization confirmation; safe phase metadata may appear in the general task audit. Task/binding/replay writes are atomic. HTTP 202 is the accepted task; 200 plus Idempotent-Replay:true is the same success replay. A reused key with a different payload returns 409. Never automatically repeat a POST after an uncertain response; explicit retry reuses its key. No endpoint, Git URL or credential is returned. After commit, dispatch failure or unknown acknowledgement still returns 202 with the accepted task_id. Read dedicated task details for actual dispatch state; never create again with a new key. Gateway or server 5xx is also an unknown outcome; explicit retry retains the original idempotency key.",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "parameters": [
                    {
                        ...
                        "type": "string",
                        "required": true,
                        "description": "Must match current aig_csrf Cookie."
                    },
                    {
                        "in": "header",
                        "name": "Idempotency-Key",
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/mcpscans.CreateRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Successful operation or Idempotent-Replay:true.",
                        "schema": {
                            "$ref": "#/definitions/mcpscans.MutationResponse"
                        }
                    },
                    "202": {
                        "description": "Safe response.",
        ...
        "/api/v1/platform/mcp-scans/{taskID}": {
            "get": {
                "summary": "Read MCP scan",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Historical MCP remains readable. No raw target/parameter/log/attachment-name fields. report_id only for an existing authorized completed snapshot.",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "responses": {
                    "200": {
                        "description": "Safe response.",
                        "schema": {
                            "$ref": "#/definitions/mcpscans.Detail"
                        }
                    },
                    "400": {
                        "description": "Invalid strict request or server-owned outbound policy rejected."
        ...
        "/api/v1/platform/mcp-scans/{taskID}/cancel": {
            "post": {
                "summary": "Cancel MCP scan",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Engine cancellation is idempotent; final task status, audit and replay response commit atomically. No automatic POST retry.",
                "tags": [
                    "MCP"
                ],
                "produces": [
        ...
        "/api/v1/platform/mcp-connection-configs": {
            "get": {
                "summary": "List safe MCP connection configurations",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. ",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "responses": {
                    "200": {
                        "description": "Safe response.",
                        "schema": {
                            "$ref": "#/definitions/mcpconnections.ListResponse"
                        }
                    },
                    "400": {
                        "description": "Invalid strict request or server-owned outbound policy rejected."
            ...
            "post": {
                "summary": "Create MCP connection configuration",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. ",
                "tags": [
                    "MCP"
                ],
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/mcpconnections.CreateRequest"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Successful operation or Idempotent-Replay:true.",
                        "schema": {
                            "$ref": "#/definitions/mcpconnections.MutationResponse"
                        }
                    },
                    "201": {
                        "description": "Safe response.",
        ...
        "/api/v1/platform/mcp-connection-configs/{configID}": {
            "get": {
                "summary": "Read editable MCP configuration",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Owner or administrator only; no-store; ETag is quoted resource_revision. Secrets remain write-only.",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "responses": {
                    "200": {
                        "description": "Safe response.",
                        "schema": {
                            "$ref": "#/definitions/mcpconnections.Detail"
                        }
                    },
                    "400": {
                        "description": "Invalid strict request or server-owned outbound policy rejected."
            ...
            "patch": {
                ...
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Version eligibility and revision checked under lock. Missing/malformed If-Match:428.",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "parameters": [
                    ...
                    },
                    {
                        ...
                        "type": "string",
                        "required": true,
                        "description": "Quoted current resource_revision; missing or malformed is 428."
                    },
                    {
                        "in": "body",
                        "name": "request",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/mcpconnections.UpdateRequest"
                        }
                    }
                ],
                "responses": {
                    ...
                    "428": {
                        "description": "Quoted If-Match resource revision required."
                    },
                    "500": {
                        "description": "Fixed safe error only; no upstream diagnostics or secrets."
                    },
        ...
        "/api/v1/platform/mcp-connection-configs/{configID}/test": {
            "post": {
                ...
                "responses": {
                    ...
                    "429": {
                        "description": "Probe rate limited."
                    },
                    "500": {
                        "description": "Fixed safe error only; no upstream diagnostics or secrets."
                    },
        ...
        "/api/v1/platform/mcp-connection-options": {
            "get": {
                "summary": "Select eligible MCP connections",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Writer roles only. Includes only visible enabled, passed current immutable versions with a concrete transport and current outbound eligibility. No endpoint/header values.",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "responses": {
                    "200": {
                        "description": "Safe response.",
                        "schema": {
                            "$ref": "#/definitions/mcpconnections.OptionsResponse"
                        }
                    },
                    "400": {
                        "description": "Invalid strict request or server-owned outbound policy rejected."
        ...
        "/api/v1/platform/mcp-scan-attachments": {
            "post": {
                "summary": "Upload MCP-only code attachment",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. MCP-only namespace. Response has opaque ID, state, bytes and configured size limits; no filename. There is no browser download endpoint.",
                "tags": [
                    "MCP"
                ],
                "produces": [
                ...
                "responses": {
                    "201": {
                        "description": "Safe response.",
                        "schema": {
                            "$ref": "#/definitions/mcpscans.AttachmentResponse"
                        }
                    },
                    "400": {
                        "description": "Invalid strict request or server-owned outbound policy rejected."
        ...
        "/api/v1/platform/mcp-scan-attachments/chunked": {
            "post": {
                "summary": "Begin MCP chunked attachment",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. ",
                "tags": [
                    "MCP"
                ],
        ...
        "/api/v1/platform/mcp-scan-attachments/{attachmentID}/chunks": {
            "post": {
                "summary": "Upload idempotent MCP chunk",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Idempotency is server-derived from upload ID, chunk index and SHA-256 digest. Exact replay returns 204; same index with different bytes returns 409. Per-upload lock serializes chunk/merge/abort/cleanup. Counter, audit and replay are atomic.",
                "tags": [
                    "MCP"
                ],
                "produces": [
        ...
        "/api/v1/platform/mcp-scan-attachments/{attachmentID}/merge": {
            "post": {
                "summary": "Merge MCP code attachment",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Validates complete bytes under per-upload resource lock. Same-key replay returns ready without remerging. No browser download route.",
                "tags": [
                    "MCP"
                ],
                "produces": [
        ...
        "/api/v1/platform/mcp-scan-attachments/{attachmentID}": {
            "delete": {
                "summary": "Abort unbound MCP attachment",
                "description": "Authenticated Cookie Subject; first-password-change gate. Ordinary users access their own resources, admins govern all, auditors read safe lists/details only and cannot mutate or obtain connection management details/options. Only uploading/ready owner-scoped attachments. Attached files cannot be removed.",
                "tags": [
                    "MCP"
                ],
                "produces": [
        ...
        "/api/v1/platform/target-credentials": {
            "get": {
                "tags": [
                    "target-credentials"
                ],
                ...
                "summary": "List own infrastructure target credentials",
                "description": "Private infrastructure target credential. Users and administrators access only their own resources; auditors cannot access them. Cookie identity, completed password change and CSRF for unsafe methods are required. Responses are Cache-Control: no-store and never expose username, secret, encryption material or headers. AES-GCM storage reuses the managed MODEL_MASTER_KEY keyring with a domain-separated AAD binding resource, owner, origin, auth type, header name and revision.",
                "responses": {
                    "200": {
                        "description": "Safe metadata including disabled entries.",
                        "schema": {
                            "type": "object",
                            "required": [
                                "items"
                            ...
                            "properties": {
                                "items": {
                                    "type": "array",
                                    "items": {
                                        "$ref": "#/definitions/targetcredentials.View"
                                    }
                                }
                            }
                        }
                    ...
                    "400": {
                        "description": "Invalid body or target; fixed safe message."
                    },
                    "401": {
                        "description": "Unauthenticated."
                    },
                    "403": {
                        "description": "Only authenticated users and administrators with completed password change may access their own credentials; writes require CSRF."
                    },
                    "404": {
                        "description": "No credential visible to this owner."
                    },
                    "500": {
                        "description": "Credential or audit unavailable; no internal details."
                    }
                }
            },
            "post": {
                ...
                "summary": "Create private infrastructure target credential",
                "description": "Private infrastructure target credential. Users and administrators access only their own resources; auditors cannot access them. Cookie identity, completed password change and CSRF for unsafe methods are required. Responses are Cache-Control: no-store and never expose username, secret, encryption material or headers. AES-GCM storage reuses the managed MODEL_MASTER_KEY keyring with a domain-separated AAD binding resource, owner, origin, auth type, header name and revision. Body limit: 16 KiB. Bearer receives the token without its prefix; API Key specifies header_name; Basic requires username and secret; Cookie uses name=value pairs. No connection probe is performed.",
                "parameters": [
                    {
                        "in": "body",
                        "name": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/targetcredentials.CreateInput"
                        }
                    }
                ],
                "responses": {
                    "201": {
                        "description": "Created; ETag contains quoted revision.",
                        "schema": {
                            "$ref": "#/definitions/targetcredentials.View"
                        }
                    },
        ...
        "/api/v1/platform/target-credentials/{id}": {
            "get": {
                "tags": [
                    "target-credentials"
                ],
                ...
                "summary": "Get own infrastructure target credential",
                "description": "Private infrastructure target credential. Users and administrators access only their own resources; auditors cannot access them. Cookie identity, completed password change and CSRF for unsafe methods are required. Responses are Cache-Control: no-store and never expose username, secret, encryption material or headers. AES-GCM storage reuses the managed MODEL_MASTER_KEY keyring with a domain-separated AAD binding resource, owner, origin, auth type, header name and revision.",
                "parameters": [
                    {
                        "in": "path",
                ...
                "responses": {
                    "200": {
                        "description": "Safe metadata with ETag.",
                        "schema": {
                            "$ref": "#/definitions/targetcredentials.View"
                        }
                    },
            ...
            "put": {
                ...
                "summary": "Replace or disable an infrastructure target credential",
                "description": "Private infrastructure target credential. Users and administrators access only their own resources; auditors cannot access them. Cookie identity, completed password change and CSRF for unsafe methods are required. Responses are Cache-Control: no-store and never expose username, secret, encryption material or headers. AES-GCM storage reuses the managed MODEL_MASTER_KEY keyring with a domain-separated AAD binding resource, owner, origin, auth type, header name and revision. Body limit: 16 KiB. Name, origin and auth_type are required. Empty secret retains the current encrypted secret only when target and authentication configuration are unchanged. Successful update increments revision, invalidating pending references; requests already issued are not recalled.",
                "parameters": [
                    {
                        "in": "path",
                        "name": "id",
                    ...
                    },
                    {
                        ...
                        "required": true,
                        "type": "string",
                        "description": "Quoted positive decimal revision, for example \"1\". Stale revision returns 409; missing or malformed header returns 428."
                    },
                    {
                        "in": "body",
                        "name": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/targetcredentials.UpdateInput"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Updated metadata with new ETag.",
                        "schema": {
                            "$ref": "#/definitions/targetcredentials.View"
                        }
                    },
                    ...
                    "409": {
                        "description": "Concurrent revision conflict; refresh before editing."
                    },
                    "428": {
                        "description": "Current If-Match revision required."
                    },
                    "500": {
                        "description": "Credential or audit unavailable; no internal details."
                    }
            ...
            "delete": {
                ...
                "summary": "Delete own infrastructure target credential",
                "description": "Private infrastructure target credential. Users and administrators access only their own resources; auditors cannot access them. Cookie identity, completed password change and CSRF for unsafe methods are required. Responses are Cache-Control: no-store and never expose username, secret, encryption material or headers. AES-GCM storage reuses the managed MODEL_MASTER_KEY keyring with a domain-separated AAD binding resource, owner, origin, auth type, header name and revision. New and pending tasks can no longer resolve the deleted credential; already-issued requests are not recalled.",
                "parameters": [
                    {
                        "in": "path",
                        "name": "id",
        ...
        "/api/v1/platform/models/test": {
            "post": {
                ...
                "summary": "Test unsaved model configuration",
                "description": "Explicit model connectivity test from the platform server. Cookie authentication, completed password change, writable-model role and CSRF are required. Token is write-only and never returned or logged. Accepts HTTP/HTTPS and ordinary intranet addresses; rejects loopback, link-local, unspecified, multicast and metadata destinations with DNS/dial validation. Sends one bounded Chat Completions request with a 30-second timeout, verified TLS, no proxy, no retry and no redirect. Response limit is 64 KiB; HTTP 200 alone is not success: a valid nonempty assistant text response is required. Probe results use HTTP 200 even for upstream authentication or network failures. Does not save models or create scan tasks. Audit contains only safe metadata. One active probe per user, five seconds between user starts; globally eight active probes and 100 milliseconds between starts, with bounded user state. Platform reachability does not guarantee Agent reachability. Drafts require a nonempty Token; name, note and limit are not required.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
                ...
                "parameters": [
                    {
                        ...
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/models.ProbeInput"
                        }
                    }
                ],
                "responses": {
                    "200": {
                        "description": "Safe success or classified error result.",
                        "schema": {
                            "$ref": "#/definitions/models.ProbeResult"
                        }
                    },
                    "400": {
                        "description": "Invalid or oversized input; no upstream request."
                    },
                    "401": {
                        "description": "Unauthenticated."
                    },
                    "403": {
                        "description": "Role, writable-model access, password-change or CSRF policy denied."
                    },
                    "404": {
                        "description": "Saved model not found."
                    },
                    "429": {
                        "description": "Probe concurrency or frequency limit reached."
                    },
                    "500": {
                        "description": "Credentials or audit unavailable; no unsafe details."
                    }
                }
            }
        },
        "/api/v1/platform/models/{modelID}/test": {
            "post": {
                "tags": [
                    "models"
                ],
                "summary": "Test current input using an authorized saved model",
                "description": "Explicit model connectivity test from the platform server. Cookie authentication, completed password change, writable-model role and CSRF are required. Token is write-only and never returned or logged. Accepts HTTP/HTTPS and ordinary intranet addresses; rejects loopback, link-local, unspecified, multicast and metadata destinations with DNS/dial validation. Sends one bounded Chat Completions request with a 30-second timeout, verified TLS, no proxy, no retry and no redirect. Response limit is 64 KiB; HTTP 200 alone is not success: a valid nonempty assistant text response is required. Probe results use HTTP 200 even for upstream authentication or network failures. Does not save models or create scan tasks. Audit contains only safe metadata. One active probe per user, five seconds between user starts; globally eight active probes and 100 milliseconds between starts, with bounded user state. Platform reachability does not guarantee Agent reachability. Stored Token reuse requires writable access and an unchanged normalized full base URL, including the API path.",
                "consumes": [
                    "application/json"
                ],
                "produces": [
    ...
    "definitions": {
        ...
        "mcpworkbench.ActiveTask": {
            "description": "Safe active MCP task projection. It has no request content, target, parameters, attachment, identity, or engine fields.",
            "properties": {
                "label": {
                    "description": "Server-generated generic label using only an opaque task ID prefix.",
                    "type": "string"
                },
                "phase": {
                    "description": "Always null until the platform has a trusted stage-event source; it is never inferred from task status.",
                    "type": "string",
                    "x-nullable": true
                },
                "source_kind": {
                    "description": "Server-decoded MCP source category; persisted legacy values fall back to legacy_unknown.",
                    "enum": [
                        "repository",
                        "service",
                        "legacy_unknown"
                ...
                "task_id": {
                    "description": "Opaque platform task identifier.",
                    "type": "string"
                },
                "updated_at": {
                    "format": "date-time",
        ...
        "mcpworkbench.Metrics": {
            "description": "Fixed MCP workbench counters for the shared thirty UTC-day window.",
            "properties": {
                "completed_30d": {
                    "minimum": 0,
                    "type": "integer"
        ...
        "mcpworkbench.RecentRisk": {
            "description": "Safe server-derived MCP risk highlight; it omits raw results, scanner text, evidence, impact, remediation, and render artifacts.",
            "properties": {
                "category": {
                    "enum": [
                        "dangerous_tool",
                ...
                "summary": {
                    "description": "Server-owned generic summary derived only from fixed category and severity; at most 160 Unicode code points.",
                    "maxLength": 160,
                    "type": "string"
                },
                "task_id": {
        ...
        "mcpworkbench.View": {
            "description": "Complete read-only safe projection for the MCP scan workbench.",
            "properties": {
                "active_tasks": {
                    "items": {
                        "$ref": "#/definitions/mcpworkbench.ActiveTask"
                    },
                    "maxItems": 10,
                    "type": "array"
                },
                "metrics": {
                    "$ref": "#/definitions/mcpworkbench.Metrics"
                },
                "recent_risks": {
                    "items": {
                        "$ref": "#/definitions/mcpworkbench.RecentRisk"
                    },
                    "maxItems": 5,
                    "type": "array"
                }
        ...
        "reports.ReportDetail": {
            "description": "Safe immutable online report. It contains only a server-safe RenderModel projection and public report metadata; stored source results, render payloads, private ownership metadata, Logo bytes, paths, and mutable brand records are absent. MCP technical findings are generic server-derived summaries without targets, repository identifiers, or scanner free text.",
            "properties": {
                "completed_at": {
                    "format": "date-time",
                    "type": "string"
        ...
        "reports.RenderModel": {
            "description": "Versioned presentation model frozen when the snapshot is created. Before online MCP detail or PDF presentation, its technical findings are projected into generic server-derived summaries.",
            "properties": {
                "completed_at": {
                    "format": "date-time",
                    "type": "string"
        ...
        "tasks.TaskInputSummary": {
            ...
            "properties": {
                ...
                "source_kind": {
                    "description": "Server-decoded MCP source category. Missing or unrecognized legacy persisted values are legacy_unknown; raw source content is never returned.",
                    "enum": [
                        "repository",
                        "service",
                        "legacy_unknown"
        ...
        "mcpscans.CreateRequest": {
            ...
            "properties": {
                ...
                "authorization_confirmed": {
                    "type": "boolean",
                    "description": "Required true only for service; repository forbids this field."
                },
                "model_id": {
                    "type": "string",
                    "format": "uuid",
                    "description": "Optional governed model. Omit for basic read-only checks without an LLM; no default-model or environment-credential fallback. Basic report coverage and reference scores are explicitly limited."
                },
                "thread": {
                    "type": "integer",
                    "minimum": 1,
            ...
            "description": "Dedicated mcp_scan only. Flat strict JSON object, exact case-sensitive keys; null, duplicates and unknown fields rejected. source_kind repository accepts exactly one HTTPS Git repository_url or 1..10 ready owner-scoped MCP attachment_ids; both service fields and authorization_confirmed must be absent. service requires connection_config_id, exact immutable connection_config_version, authorization_confirmed=true and no repository fields. Only enabled, tested and policy-approved saved connections are eligible. model_id is an optional governed reference; thread defaults to 4 and is 1..32. Language is server-fixed zh_CN; task_type, language, country_iso_code, content, headers and secrets are forbidden. Source-related audit metadata contains source_kind and the Boolean authorization confirmation; safe phase metadata may appear in the general task audit. Task/binding/replay writes are atomic. HTTP 202 is the accepted task; 200 plus Idempotent-Replay:true is the same success replay. A reused key with a different payload returns 409. Never automatically repeat a POST after an uncertain response; explicit retry reuses its key. No endpoint, Git URL or credential is returned."
        },
        "mcpscans.MutationResponse": {
            "type": "object",
            "additionalProperties": false,
            "properties": {
                "task_id": {
        ...
        "mcpscans.ListResponse": {
            ...
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/mcpscans.Summary"
                    }
                },
                "total": {
                    "type": "integer",
        ...
        "mcpconnections.CreateRequest": {
            ...
            "properties": {
                ...
                "server_url": {
                    "type": "string",
                    "format": "uri",
                    "description": "Configured HTTP(S) endpoint; validated against server-owned outbound CIDRs. No URL credentials, query or fragment."
                },
                "transport": {
                    "type": "string",
                    "enum": [
                ...
                "authentication": {
                    ...
                    "description": "Write-only authentication. bearer requires secret; api_key_header requires header_name and secret; none forbids credentials and headers; custom_headers requires at least one header."
                },
                "headers": {
                    "type": "array",
                    "items": {
            ...
            "description": "Always creates a private connection owned by the authenticated Subject. scope/owner IDs and outbound policy flags are not accepted."
        },
        "mcpconnections.UpdateRequest": {
            "type": "object",
            "additionalProperties": false,
            "properties": {
                "name": {
                ...
                "authentication": {
                    ...
                    "required": [],
                    "description": "Omitted secret retains a previously configured secret only for the same kind and header name. Kind/header-name changes require replacement. Explicit blank secret is invalid when required."
                },
                "headers": {
                    "type": "array",
                    "items": {
            ...
            "required": [],
            "description": "Partial patch. enabled must be the only field when present. Connection-material or transport changes create a new immutable version, disable it and require a new test. Metadata-only edits preserve version and eligibility. headers:[] removes headers; omitted values preserve matching names. none must remove prior headers. If-Match revision CAS required."
        },
        "mcpconnections.Summary": {
            "type": "object",
            "additionalProperties": false,
            "properties": {
                "id": {
                ...
                "resource_revision": {
                    "type": "string",
                    "pattern": "^[1-9][0-9]*$",
                    "description": "Decimal string; send quoted as If-Match, for example \"2\". Not a connection version."
                },
                "enabled": {
                    "type": "boolean"
                },
        ...
        "mcpconnections.ListResponse": {
            ...
            "properties": {
                "items": {
                    "type": "array",
                    "items": {
                        "$ref": "#/definitions/mcpconnections.Summary"
                    }
                }
            },
            "required": [
        ...
        "mcpconnections.Detail": {
            ...
            "description": "Owner/admin management-only exception: returns endpoint and configured header names for editing, but never secret/header values. No-store. Auditor has no management detail access."
        },
        "mcpconnections.MutationResponse": {
            "type": "object",
            "additionalProperties": false,
            "properties": {
                "id": {
        ...
        "mcpconnections.OptionsResponse": {
            "type": "object",
            "additionalProperties": false,
            "properties": {
                "items": {
        ...
        "targetcredentials.View": {
            ...
            "properties": {
                "id": {
                    "type": "string",
                    "description": "Opaque owner-scoped reference.",
                    "maxLength": 128
                },
                "name": {
                    "type": "string",
                    "description": "Display name of this private credential.",
                    "maxLength": 160
                },
                "origin": {
                    "type": "string",
                    "description": "Canonical target origin; HTTP and HTTPS are accepted by default. Scheme, host and optional port only, without userinfo, path, query or fragment. Default HTTPS port 443 and HTTP port 80 are normalized.",
                    "maxLength": 2048
                },
                "allow_insecure_http": {
                    "type": "boolean",
                    "description": "Derived from whether the saved origin uses HTTP. Target authentication remains restricted to the saved scheme, host and port, and HTTPS certificate verification remains enabled."
                },
                "auth_type": {
                    "type": "string",
                    "enum": [
                ...
                "header_name": {
                    "type": "string",
                    "description": "API Key header name. Host, proxy and transport control headers are forbidden; other auth types use an empty string.",
                    "maxLength": 128
                },
                "disabled": {
                    "type": "boolean"
        ...
        "targetcredentials.CreateInput": {
            ...
            "properties": {
                ...
                "allow_insecure_http": {
                    "type": "boolean",
                    "description": "Legacy optional compatibility field; its value is ignored. HTTP and HTTPS are accepted directly based on origin, including edits retaining the existing secret."
                },
                "auth_type": {
                    "type": "string",
                    "enum": [
                ...
                "username": {
                    "type": "string",
                    "description": "Write-only Basic username; provide again when replacing the Basic password.",
                    "maxLength": 256,
                    "x-write-only": true
                },
                "secret": {
                    "type": "string",
                    "description": "Write-only token, API key, Basic password or Cookie string. Never returned.",
                    "maxLength": 8192,
                    "x-write-only": true
                }
            }
        },
        "targetcredentials.UpdateInput": {
            "type": "object",
            "additionalProperties": false,
            "required": [
                "name",
            ...
            "properties": {
                ...
                "secret": {
                    "type": "string",
                    "description": "Empty or omitted retains the current secret. Changing origin, auth_type or header_name requires a new nonempty secret. Any update increments revision.",
                    "maxLength": 8192,
                    "x-write-only": true
                }
            }
        ...
        "models.ProbeInput": {
            ...
            "properties": {
                "provider_model": {
                    ...
                    "minLength": 1,
                    "maxLength": 512,
                    "description": "Model ID accepted by the inference endpoint; labeled 模型ID in the console."
                },
                "base_url": {
                    "type": "string",
                    "minLength": 1,
                    "maxLength": 2048,
                    "description": "HTTP/HTTPS API base URL, such as http://inference.internal:8000/v1; no userinfo, query or fragment."
                },
                "token": {
                    "type": "string",
                    "maxLength": 8192,
                    "x-write-only": true,
                    "description": "Write-only API key, without Bearer prefix. Required for drafts. On saved models, omitted or empty reuses the stored Token only when the normalized full base URL is unchanged. Masked tokens are not accepted."
                }
            }
        },
        "models.ProbeResult": {
            ...
            "properties": {
                ...
                "message": {
                    "type": "string",
                    "description": "Fixed safe result description; never upstream text, credentials or request diagnostics. The console maps code to local copy."
                },
                "elapsed_ms": {
                    "type": "integer",
                    "format": "int64",
                    "minimum": 0,
                    "description": "Elapsed probe time in milliseconds."
                }
            }
        }
    }
