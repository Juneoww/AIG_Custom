...
- `GET /api/v1/platform/admin/audit-events`: administrators and auditors query append-only, sanitized events. Filters include `action`, `actor_user_id`, `resource_type`, `resource_id`, and `limit`. Governed mutations first append a durable `pending` event and correlate the later `success` or `failure` event with the same `request_id`. A completed mutation is durably queued before its final audit append, so an append outage does not change the successful business response.
- `POST /api/v1/platform/admin/audit-events/prepared/{requestID}/finalize`: administrators explicitly recover a prepared file-backed mutation after verifying whether the legacy file change succeeded or failed. The body is `{"outcome":"success|failure","metadata":{...}}`. Recovery promotes the existing request to the same stable outbox/event identity and never repeats the file mutation. Retrying the same outcome is idempotent; a conflicting outcome returns `409`. Use the `request_id` from the audit-event query or the safe `X-Audit-Request-ID` response header; auditors and users cannot finalize.
- `POST /api/v1/platform/admin/audit-events/reconcile`: administrators retry durable completion-outbox entries. Stable event IDs make repeated reconciliation idempotent; the response reports the number reconciled.
...
Only `aig migrate` may apply database DDL. After the versioned schema reaches v4, the command atomically imports any rows from the legacy plaintext `models` table into encrypted platform storage and deletes those plaintext rows in the same PostgreSQL transaction. A non-empty legacy table requires `MODEL_MASTER_KEY_ID` and a valid 32-byte base64 `MODEL_MASTER_KEY`; an empty legacy table allows schema-only migration without model keys. Production `webserver` startup still requires the active model key. Runtime datastore initialization performs read-only schema, index, and legacy-row validation; an old/incomplete schema or any remaining plaintext legacy row fails closed with an instruction to run `aig migrate` and never creates or alters tables.
