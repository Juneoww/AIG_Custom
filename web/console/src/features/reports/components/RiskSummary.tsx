/**
 * 功能：展示不可变报告的评分、风险分布和重点风险。
 * 实现：使用语义定义列表，并按高、中、低及原始次序稳定排列风险项。
 * 输入：已校验的风险摘要、评分说明、映射版本和 Top 风险。
 * 输出：可访问且适配长中文的风险摘要区域。
...
const badgeColors: Record<RiskSeverity, 'danger' | 'warning' | 'success'> = {
  high: 'danger', medium: 'warning', low: 'success',
}

const useStyles = makeStyles({
  root: { padding: tokens.spacingVerticalL, boxShadow: 'none' },
...
export function RiskSummary({ risk, explanation, mappingVersion, topRisks }: RiskSummaryProps) {
  ...
  return (
    <Card className={styles.root} role="region" aria-label="风险摘要">
      <h2 className={styles.title}>风险摘要</h2>
      ...
      <div className={styles.distribution} aria-label="风险分布">
        ...
        <Badge color="success">低风险 {risk.low}</Badge>
      </div>
