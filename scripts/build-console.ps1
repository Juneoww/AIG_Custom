...
# 实现：容器内执行冻结依赖安装、字体准备、质量检查和 Vite 构建；临时目录校验后以同卷移动替换。
...
$dockerArguments = @(
    ...
    "sh",
    "-ec",
    "corepack enable && pnpm install --frozen-lockfile && pnpm run prepare:fonts && pnpm lint && pnpm typecheck && pnpm test:run && pnpm build"
)
