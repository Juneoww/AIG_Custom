...
本文面向侧栏中的 MCP 扫描、Skills 扫描、AI 基础设施扫描、Agent 工作流扫描四类业务。以 AI 基础设施扫描的已实现代码为参照，为其余三类（MCP 扫描、Skills 扫描、Agent 工作流扫描）的产品、前端、后端与测试开发约定专属工作台的页面组织、数据合同、治理边界和交付要求。
...
Skills ZIP 首期在 `codex/skills-scan` 分支基于 `ddb045545` 补齐，见[第 11 节](#11-skills-zip-首期接入)。第 1–10 节保留 AI 工作台合并时的历史快照；其中 Skills 的待定项由第 11 节的明确合同替代，不能据此推断历史基线已有 Skills 能力。
...
| Skills 扫描 | 尚无独立标准类型，映射待接入设计确认 | 底层能力复用与类型映射待核对 | 已有侧栏入口；专属任务身份、输入合同及工作台尚需补齐，不能仅凭入口认定已接通。 |
| AI 基础设施扫描 | `ai_infra_scan` | `AI-Infra-Scan` | 已有专属列表、新建和详情路由。 |
| Agent 工作流扫描 | `agent_scan` | `Agent-Scan` | 有通用创建与适配合同；专属页及实际引擎能力需逐项验收。 |
...
四类专属工作台应复用现有平台任务服务、鉴权、附件管理、幂等创建、调度、取消和报告快照链路。专属页面负责表达各自的业务输入与安全摘要；Skills 尚未贯通的合同与适配需要补齐，不能视为已实现。
...
其他类别采用“专属列表 / 新建 / 详情”三条路由。MCP 扫描、Skills 扫描、Agent 工作流扫描可分别考虑以 `/tasks/mcp`、`/tasks/skills`、`/tasks/agent-workflow` 为路由前缀，但必须先核对当时路由与并行开发成果，不能把这些建议直接当成已实现路径。
...
| `GovernedModelSelector` | 单个受治理模型的分页选择与可用性验证 | 当前是可选单选；Agent 工作流扫描的必选裁判模型不能照搬“不使用模型”，Skills 的模型需求须按实际引擎合同确认。 |
...
Skills 当前不在已有标准任务类型中，不能仅向 `fixedTaskType` 传入一个新字符串完成接入。必须同步贯通前端类型与响应解析、服务端创建和筛选校验、引擎适配及报告分类；如复用既有类型，则需有明确且可持久化、可在服务端筛选的业务区分字段。

后续要求：在下一类专属工作台落地时提取确实共用的页面框架、文案和路由配置；类型专属表单独立维护。避免持续扩大同一个页面中的条件分支，也避免整份复制后让状态口径、权限或重试行为各自漂移。具体文件拆分由当时代码规模决定。
...
可用性分为 `available / pending / unavailable`：已选模型尚在验证、目录刷新或加载失败时，不能把旧缓存当成已确认可用；保留待确认选择并提供重试。确认停用或完成目录遍历仍找不到时清除选择。AI 允许不使用模型，不能据此推断 Agent 工作流扫描也允许缺失必选裁判模型；Skills 是否需要模型须在其接入合同中明确。
...
这里的其他三类明确指 MCP 扫描、Skills 扫描、Agent 工作流扫描。下表中 MCP 与 Agent 工作流的参数来自基线已有服务端合同；Skills 尚无独立平台合同，需要先完成接入设计。页面可以逐步暴露已有可用能力，不能在没有后端支持的情况下创造参数。

| 类别 | 扫描对象/执行说明 | 现有 `params` 主合同 | 专属化时的重点 |
| --- | --- | --- | --- |
| MCP 扫描 | 根据实际支持的代码、仓库或服务扫描模式定义输入；现有 `content` 与附件按 MCP 语义进入引擎 | 可选 `model_id`；可选 `thread`，1–1,024 | 先确认实际模式和附件格式；保留并发与模型治理，不能套用 IP 目标计数或端口模式。 |
| Skills 扫描 | 围绕技能定义、技能包及其相关资源梳理扫描对象；实际支持的来源、格式与执行方式待核对 | 当前尚无独立 Skills 参数合同；不得直接复用其他类别参数作为已支持能力 | 先确认底层能力与任务身份，再落实输入校验、模型需求、安全摘要、报告及独立筛选，避免与 MCP 任务混在同一专属列表。 |
| Agent 工作流扫描 | 智能体配置引用与工作流执行说明 | 必选 `agent_id`、`eval_model_id` | 从“凭证配置 → 智能体配置”对应治理入口选择智能体，从模型配置目录选择裁判模型；定义可用性与权限校验；不要把配置原文、凭据或内部运行数据直接带进任务页面。 |

Agent 目录选择器不能简单改造模型 ID 文案代替，应对照实际 Agent 目录接口及权限单独实现。页面统一使用“Agent 工作流扫描”，对接已有合同仍使用 `agent_scan`，不能因文案调整擅自改写已存任务类型。

### 8.1 Skills 扫描接入前须明确的事项

1. 盘点已有技能相关扫描能力，明确支持的对象来源、附件格式、文件限制、静态分析或执行边界；未核实的能力不能作为页面选项开放。
2. 明确使用独立 `task_type`，还是在复用底层能力时增加明确的业务区分机制。这是待确认的实现方案，不是已有 API。无论选择哪种，创建、存储、服务端分页筛选、详情类型校验和报告都必须保留 Skills 身份，不能靠浏览器本页过滤区分 MCP 与 Skills。
3. 贯通前端类型、路由和响应白名单，服务端参数校验、引擎适配和安全 DTO，并同步 API 文档与测试；确有数据库结构变化时才追加迁移。
4. 定义 Skills 专属安全摘要与指标来源，不能把 AI 的 IP `target_count` 直接解释为技能数量。底层引擎尚不支持的模式应明确不可用，不能制造已执行或已完成的展示。
...
| 项目 | 该类需要填写的内容 |
| --- | --- |
| 名称与身份 | 产品名称、标准 `task_type` 或明确的业务区分机制、引擎类型、现有实现位置。 |
...
按每类业务调整目标用例；MCP 扫描、Skills 扫描、Agent 工作流扫描不需要照抄 IP 语法测试，但必须验证自己的真实输入规则。Skills 还需验证与 MCP 的分类隔离，包括服务端分页总数、详情访问和报告归类。
