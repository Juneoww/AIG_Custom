...
**Goal:** 在保留 AI-Infra-Guard 扫描引擎及规则判定的前提下，交付具备本地账号登录、三角色治理、管理层报告、独立品牌 UI 和麒麟/达梦部署能力的单租户企业安全平台。
...
**Tech Stack:** Go 1.23、Gin、GORM/`database/sql`、PostgreSQL（开发/测试）、达梦（生产）、React、TypeScript、Vite、Vitest、Playwright、Docker Buildx、中文字体/PDF 渲染运行时。
...
- 达梦驱动、目标版本和 GORM 兼容性先以实际容器验证。若供应商驱动不能提供可靠 GORM 方言，平台新表改用边界清晰的 `database/sql` repository；禁止伪装成 PostgreSQL/MySQL 方言上线。

## 计划文件结构

| 文件 | 职责 |
| --- | --- |
| `pkg/database/config.go`、`pkg/database/migrate.go` | 显式选择开发 PostgreSQL、生产达梦和迁移入口；去除 SQLite 专有启动假设。 |
...
| `Dockerfile*`、`docker-compose*.yml`、`deploy/*` | 多架构、离线导入、麒麟和达梦部署配置。 |
...
Run: `powershell -ExecutionPolicy Bypass -File scripts/check-attribution.ps1`
...
Run: `go test ./pkg/database ./common/websocket; powershell -ExecutionPolicy Bypass -File scripts/check-attribution.ps1`
...
### Task 2: 数据库驱动、迁移与达梦兼容性验证

**Files:**
- Modify: `pkg/database/config.go`, `pkg/database/database_test.go`
- Create: `pkg/database/migrate.go`, `pkg/database/migrate_test.go`, `pkg/database/testutil_test.go`
- Create: `deploy/compose/docker-compose.postgres.yml`, `deploy/compose/docker-compose.dm.yml`, `docs/deployment/dameng.md`

- [ ] **Step 1: 写入失败测试：数据库配置必须显式区分 `postgres`、`dameng` 与测试驱动**

在 `pkg/database/migrate_test.go` 覆盖缺少 DSN、未知驱动、PostgreSQL DSN 和仅本地测试 SQLite 临时库；测试不得依赖生产环境变量。

- [ ] **Step 2: 运行数据库配置测试并确认失败**

Run: `go test ./pkg/database -run 'Test(DatabaseConfig|Migration)' -count=1`

Expected: FAIL，尚无驱动选择和平台迁移入口。

- [ ] **Step 3: 实现显式数据库配置和版本化迁移**

扩展 `Config` 为 `Driver`、`DSN`、连接池参数；新增 `Migrate(db)`，用迁移版本表而不是仅依赖 `AutoMigrate`。保留读取旧 SQLite 数据的迁移工具，但生产启动拒绝 SQLite。先从实际交付环境取得并记录 `DM_IMAGE`、`DM_VERSION` 与供应商支持的 Go 驱动模块/版本；未提供这三项不得猜测公开镜像或驱动。随后用该驱动完成 `database/sql` 连通性小验证，并将已验证的驱动版本锁入 `go.mod`。

- [ ] **Step 4: 为 PostgreSQL 和达梦编排最小验证服务**

新增 profile/独立 compose，传入 `DB_DRIVER`、`DB_DSN` 和由部署环境提供的 `DM_IMAGE`；在 `docs/deployment/dameng.md` 固化已验证达梦版本、镜像来源、驱动版本、初始化账号权限与回滚/备份要求。

- [ ] **Step 5: 运行测试和容器迁移验证**

Run: `go test ./pkg/database -count=1; docker compose -f deploy/compose/docker-compose.postgres.yml run --rm migrate; $env:DM_IMAGE='<vendor-approved-image>'; docker compose -f deploy/compose/docker-compose.dm.yml run --rm migrate`

Expected: 单测通过；两种数据库均创建同一迁移版本且可回读。

- [ ] **Step 6: Commit**

```bash
git add pkg/database go.mod go.sum deploy/compose docs/deployment/dameng.md
git commit -m "feat: add postgres and dameng migration support"
```
...
覆盖 Argon2id 校验、错误密码、禁用账号、首次/重置后强制改密、会话轮换/撤销、CSRF 缺失，以及 `admin`、`user`、`auditor` 的最小权限矩阵。

- [ ] **Step 2: 运行身份测试并确认失败**

Run: `go test ./internal/platform/identity -count=1`

Expected: FAIL，包不存在。

- [ ] **Step 3: 实现安全身份服务**

实现用户、角色、会话与密码重置状态实体；密码只存 Argon2id 哈希。将随机会话 token 的哈希入库，Cookie 设置 `HttpOnly`、`Secure`、`SameSite=Lax`，状态变更请求使用双提交 CSRF token。新增 `aig bootstrap-admin`，只在无管理员时创建首个管理员，密码经 stdin 或一次性环境变量读取且不写日志。
...
Run: `go test ./internal/platform/identity ./common/websocket -count=1`
...
- Modify: `common/websocket/model_api.go`, `common/websocket/server.go`, `api.md`, `api_zh.md`, `docs/swagger.yaml`

- [ ] **Step 1: 写入失败测试：管理员用户/角色变更必须审计，模型密钥不能出现在 API 响应**

测试管理员创建用户、分配角色、禁用、重置密码；审计员不能写；普通用户无法读取别人私有模型；全局模型仅管理员可写；模型 token 在列表、详情、日志序列化中永不明文出现。

- [ ] **Step 2: 运行测试并确认失败**

Run: `go test ./internal/platform/admin ./internal/platform/audit ./internal/platform/models -count=1`

Expected: FAIL，包不存在。

- [ ] **Step 3: 实现后台与审计服务**

实现受控的用户、角色、密码重置、审计查询 API；审计事件包括登录成功/失败、角色与账号变更、任务变更、报告导出和系统配置变更。用由环境变量注入、可轮换的主密钥加密模型 token；正常用户仅管理本人私有模型，管理员管理全局模型并仅看密钥掩码。

- [ ] **Step 4: 迁移并替换旧模型 API 的信任来源**

将现有 `model_api.go` 变为内部兼容实现或由平台 handler 调用，删除请求头用户名依赖；路由与 Swagger 文档标记旧端点为 internal/deprecated。

- [ ] **Step 5: 运行 API 回归测试**

Run: `go test ./internal/platform/... ./common/websocket -count=1`
...
```bash
git add internal/platform/admin internal/platform/audit internal/platform/models common/websocket/model_api.go common/websocket/server.go api.md api_zh.md docs/swagger.yaml
...
测试普通用户仅能读取/取消本人任务，审计员全局只读，管理员可治理；相同幂等键不会重复提交；分发临时失败保留状态而不删除任务；附件 URL 不能由未授权用户读取。
...
Run: `go test ./internal/platform/tasks ./common/websocket -run 'Test(PlatformTask|AgentAuth|TaskOwnership)' -count=1`

Expected: FAIL，旧实现仍信任 header、默认共享并会清理失败任务。

- [ ] **Step 4: 实现适配器与状态机**

平台先事务保存任务、所有者、附件引用、幂等键和 `pending` 状态，再通过 `EngineAdapter.SubmitTask/GetTaskStatus/GetResult/CancelTask` 调用现有 `TaskManager`。仅适配器可解密模型 token 并发送给内部 Agent；所有日志使用密钥掩码。回调/轮询按平台任务 ID 更新状态，网络重试有上限且不会自动重跑已进入扫描的任务。
...
Run: `go test ./internal/platform/tasks ./common/websocket -count=1`
...
Run: `go test ./internal/platform/reports ./internal/platform/brand -count=1`
...
Run: `go test ./internal/platform/reports ./internal/platform/brand -count=1; go test ./internal/platform/tasks -count=1`
...
- [ ] **Step 3: 初始化 React/TypeScript/Vite 工程和设计令牌**

实现企业工作台式浅色高密度样式、蓝色主题令牌、独立几何 SVG 默认标识（不复用 AIG/Tencent 视觉）、可配置名称和 Logo。将构建产物输出到 `common/websocket/static`，由 Go `embed` 提供 SPA 回退。

- [ ] **Step 4: 实现 API 客户端基础**

所有请求使用安全 Cookie 和 CSRF header，统一处理 401/403/首次改密状态；禁止在 `localStorage` 存储会话或模型密钥。

- [ ] **Step 5: 扩展归属检查并运行前端单元测试与生产构建**

扩展 `scripts/check-attribution.ps1`，使其同时校验 `web/console/src/pages/AboutPage.tsx` 的上游归属文案。随后运行：`pnpm --dir web/console test --run; pnpm --dir web/console build; powershell -ExecutionPolicy Bypass -File scripts/check-attribution.ps1; go test ./common/websocket -count=1`
...
```bash
git add web/console common/websocket/static common/websocket/server.go .gitignore
...
覆盖管理员创建普通用户/审计员、首次改密、审计员无法看到写按钮、普通用户访问管理员 URL 显示 403、品牌修改只影响当前导航。

- [ ] **Step 2: 运行 E2E 并确认失败**

Run: `pnpm --dir web/console exec playwright test e2e/auth-rbac.spec.ts`

Expected: FAIL，页面不存在。

- [ ] **Step 3: 实现身份和后台页面**

完成登录、强制改密、个人中心以及用户、角色、审计、品牌设置。页面仅依据服务端 `/me` 权限显示入口，所有按钮仍由服务端授权二次保护。

- [ ] **Step 4: 运行 UI 和 API 回归**

Run: `pnpm --dir web/console test --run; pnpm --dir web/console exec playwright test e2e/auth-rbac.spec.ts; go test ./internal/platform/... -count=1`
...
覆盖普通用户创建并只看到本人任务、审计员看到全局只读数据、任务完成后生成在线报告、PDF 下载触发审计、历史快照不随品牌更新改变。

- [ ] **Step 2: 运行 E2E 并确认失败**

Run: `pnpm --dir web/console exec playwright test e2e/tasks-reports.spec.ts`

Expected: FAIL，页面和查询状态尚未实现。

- [ ] **Step 3: 实现工作台页面**

实现安全总览、扫描创建/列表/详情/实时状态、报告列表/在线详情/PDF、全局与个人模型、规则与知识库页面。风险评分和趋势只消费报告 API 预计算字段；审计员页面全程只读。

- [ ] **Step 4: 运行前后端回归**

Run: `pnpm --dir web/console test --run; pnpm --dir web/console exec playwright test e2e/tasks-reports.spec.ts; go test ./internal/platform/... ./common/websocket -count=1`
...
- Create: `docs/deployment/kylin-dameng.md`, `docs/deployment/offline-install.md`, `docs/deployment/upgrade.md`
- Test: `scripts/verify-image-platforms.ps1`

- [ ] **Step 1: 写入失败的镜像与配置验证脚本**

校验 compose 不再默认 SQLite/公开旧端口、环境变量含达梦连接/主密钥/内部 Agent token、镜像清单含 `linux/amd64` 与 `linux/arm64`、镜像中存在中文字体和非 root 运行用户。

- [ ] **Step 2: 运行验证并确认失败**

Run: `powershell -ExecutionPolicy Bypass -File scripts/verify-image-platforms.ps1`

Expected: FAIL，现有镜像使用 SQLite、Alpine server 和公开旧服务编排。

- [ ] **Step 3: 改造部署制品**

采用适合麒麟的可复现 Linux 基础镜像，前端在 builder 阶段构建；服务和 Agent 使用最小权限、显式内部网络和健康检查。提供 Buildx 多架构构建、`docker save/load` 离线导入脚本、达梦持久化与升级备份文档；第三方模型仅来自可配置的内网/私有 Base URL。

- [ ] **Step 4: 构建并验证部署制品**

Run: `New-Item -ItemType Directory -Force dist | Out-Null; docker buildx build --platform linux/amd64,linux/arm64 -f Dockerfile --output type=oci,dest=dist/aig-server.oci.tar .; docker buildx build --platform linux/amd64,linux/arm64 -f Dockerfile_Agent --output type=oci,dest=dist/aig-agent.oci.tar .; powershell -ExecutionPolicy Bypass -File scripts/verify-image-platforms.ps1; docker compose -f deploy/compose/docker-compose.production.yml config`

Expected: 通过镜像/配置验证，Compose 配置可解析。

- [ ] **Step 5: 导入离线归档并在麒麟 + 达梦验收环境冒烟**

在目标架构主机执行 `deploy/offline/import-images.sh`（输入 Task 10 Step 4 生成的 OCI 归档），将镜像按 `docker-compose.production.yml` 使用的确切标签导入本地镜像仓库；脚本须先以 `docker image inspect` 验证两个标签都存在。随后运行：`docker compose -f deploy/compose/docker-compose.production.yml up -d; docker compose -f deploy/compose/docker-compose.production.yml ps`

Expected: 平台、受控 Agent 和达梦连接健康；首次管理员登录、建任务和 PDF 导出成功。
...
git commit -m "feat: add kylin and dameng deployment support"
```
...
列出三角色矩阵、任务所有权、审计事件、报告不可变性、品牌配置、PostgreSQL/达梦迁移、双架构镜像、麒麟离线安装和既有扫描回归的可验证证据。

- [ ] **Step 2: 运行全量自动化校验**

Run: `go test ./...; pnpm --dir web/console test --run; pnpm --dir web/console exec playwright test; go build -o yamlcheck ./cmd/yamlcheck; ./yamlcheck data/fingerprints data/vuln data/vuln_en`

Expected: PASS。

- [ ] **Step 3: 运行安全回归检查**

Run: `go test ./internal/platform/... ./common/websocket -run 'Test(Unauthenticated|Unauthorized|AgentAuth|TaskOwnership|NoSecretLeak)' -count=1`

Expected: PASS；旧的无认证路径、默认共享、明文 token 日志和重复 Agent 接管均不可复现。

- [ ] **Step 4: 更新操作文档与变更日志**

将所有新 API、管理员引导、角色边界、达梦/麒麟部署、离线升级、归属声明和回滚步骤写入中英 API 文档、Swagger、README 和验收文档。
...
2. 达梦验证环境：执行全量迁移与核心读写，记录目标版本与驱动版本。
3. 麒麟 Linux：离线导入 `amd64` 或 `arm64` 对应镜像，启动 Compose 后完成同一验收流。
4. 扫描兼容性：以固定 fixture 对比改造前后 AI Infra、MCP、Agent 和 Prompt/Jailbreak 任务的引擎结果类型与关键结论。
