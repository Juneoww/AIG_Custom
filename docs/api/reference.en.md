...
`GET /api/v1/platform/tasks/{taskID}` returns `TaskDetail`, whose `input_summary` contains only bounded display metadata. Only `ai_infra_scan` and `skills_scan` details may include `model_id`: it is an opaque persisted/validated model reference for recovering the current model-catalog label. Deleted, disabled, or user-invisible models must render using the safe ID fallback. This field is not a Token, Base URL, credential, raw parameter object, or current-availability claim; raw params, nested credentials, and model objects remain absent. A Skills summary contains only `language`, `model_id`, and `scan_mode` fixed to `static`; other types omit `scan_mode`, and Skills omit names, file/skill counts, and `target_count`. A user sees only their own task; auditors/admins have global read access; absent or user-invisible tasks return `404`. `GET /api/v1/platform/tasks/{taskID}/result` is retired: after authentication and the password-change gate it always returns `410 Gone` and never reads engine output.

Only an `agent_scan` input summary may additionally include `agent_id` and `eval_model_id`. These are safe projections of persisted references, not claims of current catalog or network availability. The server validates the complete parameter contract first, then omits invalid, overlong, credential-like, or path-like historical references; it never extracts display values from inline configuration. Agent names resolve in the task owner's context, with the safe ID as a fallback when the current catalog cannot be confirmed; an administrator's same-named configuration must not replace it. Detail may include a trimmed `remark` of at most 2,000 Unicode code points. Blank or invalid historical remarks are omitted, and task lists never include remarks.
...
`POST /api/v1/platform/mcp-scans` takes a flat strict JSON object up to 64 KiB. MCP requires `source_kind` as `repository` or `service`; task type, language, content, Token and Header inputs are forbidden. Language is server-fixed `zh_CN`. Optional `model_id` is a governed reference; `thread` defaults to 4 and is 1–32.

Omitting `model_id` runs basic checks without an LLM, default-model fallback or model credentials from the environment. Repository checks only read code; service checks inspect protocol and tool metadata through the task gateway without invoking service tools. Supplying a governed model enables model-assisted analysis. Basic reports explicitly limit coverage and identify their scores as rule-based references, not comprehensive security assessments or certification.

- `repository` accepts a strict Git reference or ready code attachment, exclusively: an allowlisted HTTPS `repository_url` or 1–10 owner-scoped MCP `attachment_ids`. Service fields and `authorization_confirmed` must be absent, even if empty.
- `service` requires `authorization_confirmed=true` and permits no attachment. Supply an eligible `connection_config_id` and exact immutable `connection_config_version`; it must be visible, tested, enabled and currently allowed by server policy. No endpoint or credentials are entered on the task page.

Creation returns `202 {"task_id":"<opaque-id>","status":"pending"}`. Exact same-key replay returns 200 and `Idempotent-Replay: true`; changed payload returns 409. Writes require CSRF; task create/cancel, configuration mutations and attachment merge also require `Idempotency-Key`. Transport failures and successful responses whose body cannot be read/validated are unknown outcomes. Never automatically repeat POST; explicit retry retains the original key.

After the creation transaction commits, dispatch failure or unknown acknowledgement still returns 202 with the original `task_id`. Do not resubmit with a new key. This is an acceptance snapshot; read the dedicated MCP detail endpoint for the actual dispatch state.

Gateway or server 5xx responses do not prove that a transaction was rolled back. Treat them as unknown outcomes and retain the same idempotency key. Explicit 4xx validation, authorization and version rejections retain their defined handling.

| Endpoint | Purpose |
| --- | --- |
| GET /api/v1/platform/mcp-scans | Safe MCP history; page 1–1000, page_size 1–100, optional status |
| GET /api/v1/platform/mcp-scans/{taskID} | Safe dedicated detail; existing authorized report_id only |
| POST /api/v1/platform/mcp-scans/{taskID}/cancel | Empty object; atomic status/audit/replay; task_id/status response |
| GET/POST /api/v1/platform/mcp-connection-configs | Safe configuration list / private configuration creation |
| GET/PATCH /api/v1/platform/mcp-connection-configs/{configID} | Owner/admin editable detail / conditional update |
| POST /api/v1/platform/mcp-connection-configs/{configID}/test | Minimal initialize probe, never tool execution |
| GET /api/v1/platform/mcp-connection-options | Eligible opaque connection IDs/versions and safe labels |

Configuration fields: required `name` (80 characters), optional `description` (500), `server_url`, `transport` (auto/http/sse), `authentication.kind` (none/bearer/api_key_header/custom_headers), and conditional `secret`, `header_name` or `headers[{name,value}]`. none forbids headers; custom_headers requires at least one. Client scope/owner, proxy and TLS-bypass controls are forbidden.

Editable GET is the sole owner/admin exception returning endpoint and configured header names, never values; it is no-store. Auditors can read safe lists/tasks/reports, not editable configuration details or selection options. PATCH/test require quoted decimal-string `resource_revision` in `If-Match`, distinct from connection version; missing/malformed is 428, conflict is 409. Omitted secrets retain existing material only for unchanged authentication kind/header name. Changed kind/name requires replacement; `headers:[]` clears headers. Material changes create an immutable new version and disable it; metadata-only edits preserve eligibility. Testing first disables/clears previous eligibility, performs minimal initialize, then atomically stores a safe passed/failed result, audit and replay. A real upstream failure returns HTTP 200 with status failed without diagnostics. Minimum interval is one minute (429). A passed test must be enabled separately using an exclusive `{"enabled":true}` PATCH.

Attachments use only `/api/v1/platform/mcp-scan-attachments`: POST a file, POST `/chunked`, POST `/{attachmentID}/chunks` with chunk_index/chunk, POST `/{attachmentID}/merge` with total_chunks/file_size, or DELETE `/{attachmentID}` to abort unbound uploads. Responses contain opaque ID/state/size/server limits, never original names. No browser download endpoint exists. Chunk replay is upload-ID/index/digest based; different bytes conflict. An existing chunk without confirmed replay fails closed and is not overwritten; abort and restart that upload.

Source-related audit metadata contains source kind and the Boolean authorization confirmation; the general task audit flow may retain safe phase metadata. Lists/details/reports omit endpoints, repository URLs, secrets, raw parameters and original attachment names. Historical MCP without trusted source data remains read-only through dedicated detail as `legacy_unknown`, and is not redispatched.

Runtime configuration uses independent `MCP_CONNECTION_MASTER_KEY_ID` / `MCP_CONNECTION_MASTER_KEY` (base64 32-byte key), optional `MCP_CONNECTION_PREVIOUS_MASTER_KEYS`, `MCP_OUTBOUND_ALLOWED_CIDRS`, and separate `MCP_GIT_OUTBOUND_ALLOWED_CIDRS` / `MCP_GIT_ALLOWED_HOSTS`; empty allowlists deny access. Set `MCP_GATEWAY_BASE_URL` to a trusted address reachable by remote Agents (Compose defaults to http://webserver:8088). TLS uses system trust; environment proxies, redirects, special-use addresses and Git submodules are denied. The platform sends only short-lived capability or opaque archive references; model/MCP runtime configuration crosses a private stdin channel. Internal gateway/archive routes require `AIG_AGENT_TOKEN`, not browser identity. Archives expire within 15 minutes and permit at most 10,000 files, 16 MiB per file and 64 MiB total/archive. Process restart invalidates references.
...
**Breaking change:** `POST /api/v1/platform/tasks` no longer returns the former internal task `View`. A successful `202` now returns `TaskDetail` with required `id`, display-safe `owner`, canonical `task_type`, `status`, `created_at`, `updated_at`, and bounded `input_summary`, plus optional safe `remark`; `report_id` is always omitted. This prevents stored request and engine internals from crossing the browser boundary. Lists retain their six safe fields and omit remarks.
...
`agent_scan` dynamically scans one configured Agent. A new task requires non-whitespace `content` containing valid UTF-8 execution instructions, limited to 32 KiB (32,768 bytes). These instructions reach information collection, vulnerability detection, and vulnerability review. This API does not import workflow files or repositories. Omit `attachment_ids` or send `[]`; non-empty attachments are rejected before persistence. Optional `remark` is stored separately, trimmed, and limited to 2,000 Unicode code points. It never enters the engine, audit metadata, or report snapshot. The content byte limit differs from the remark code-point limit.

`params` accepts only the required `agent_id` and `eval_model_id` references. Select the Agent through `/api/v1/knowledge/agent/names` and the model through the governed catalog; catalog confirmation is not a successful connectivity test. Provider configuration is checked before persistence and again at dispatch and must describe a single target. HTTP/HTTPS, WebSocket, and Dify adapters are currently allowed. Dify requires explicit `config.apiKey`, `config.apiBaseUrl`, and `config.extra.dify_type` (`chat` or `workflow`); conflicting routing fields such as `url`, `endpoint`, `method`, `body`, `headers`, and `message_template` are rejected. Unknown modes, multiple targets, duplicate keys, YAML aliases, ambiguous implicit scalars, malformed configurations, and invalid field types are rejected. Coze remains unverified and is currently rejected. An allowed adapter does not establish compatibility with every external product version.

Historical requests using the same `Idempotency-Key` are compared with the persisted payload before applying the new-only instruction/attachment constraints, so existing tasks with empty instructions or attachments can still be acknowledged. Changing instructions, remark, Agent, or model creates a different payload and requires a new logical submission. Never automatically resubmit a task whose dispatch is uncertain.

In platform mode, `eval_model_id` supplies the model for the whole scan: the primary and thinking/coding auxiliary models all use that governed configuration. An `agent-security-report@1` result requires all three stages to complete, structurally complete vulnerability blocks, and the explicit final marker `<review_complete>true</review_complete>`. Zero findings also require `<no_findings>true</no_findings>`. Empty text, truncated review, connectivity failures, model errors, or exhausted iterations cannot represent a safe result. The executor publishes the report only after a successful subprocess exit, no error event, and exactly one valid result; failure or cancellation publishes no successful report.

The IDs below are placeholders. The request also requires a valid session, CSRF token, and `Idempotency-Key`:

```json
{
  "task_type": "agent_scan",
  "content": "Check the test support Agent for data leakage, tool misuse, and authorization boundaries.",
  "remark": "Pre-release retest",
  "country_iso_code": "en",
  "attachment_ids": [],
  "params": {
...
This feature reuses the existing task, remark, and report snapshot storage; it adds no task table or database migration.

### Skills ZIP static scanning

`skills_scan` is an independent platform type mapped internally to `Skills-Scan`; task filtering and reports retain the Skills identity. Submit exactly one ready ZIP previously uploaded by the current user. `params` contains only the required `model_id` of an available governed model; unknown parameters are rejected. `content` must be omitted or an empty string. This initial mode has no URL, repository, custom audit prompt, concurrency, port, or dynamic-scan options. The Chinese Skills UI explicitly sends `country_iso_code: "zh_CN"`; the API retains its existing empty, `zh`, `zh_CN`, and `en` values.
...
```json
{
  ...
  "remark": "Static audit of a Skill before release"
}
```

The ZIP limits are 20 MiB compressed, 100 MiB of actual expanded bytes, 5 MiB per file, and 2,000 original ZIP entries and at most 2,000 normalized files/directories, including implicit directories. Exactly one `SKILL.md` must exist at the ZIP root or within one top-level wrapper directory, with all files under that Skill root. `SKILL.md` must be valid UTF-8 with an object YAML frontmatter containing non-empty string `name` and `description` values, limited to 128 and 2,000 Unicode code points respectively. Documentation-only Skills are supported. Traversal, absolute paths, backslashes, links, special files, duplicate or case-conflicting paths, file/directory conflicts, duplicate YAML keys, encryption, unsupported compression, and ZIP/CRC damage are rejected; limits apply to the actual expanded streams rather than metadata alone.

Creation inspects the attachment without extracting target files into the Web service directory and binds the attachment atomically with the task. A matching idempotent retry returns the persisted task before checking whether its attachment is still ready. The executor validates and extracts again in a task-specific temporary directory and cleans it after completion or cancellation. Static tools permit only root-confined, output-bounded reading, directory listing, and search; package code, dependency installation, and arbitrary external targets cannot be executed or accessed. Every model stage uses the selected governed model. Model, parsing, or result-event failures fail the task rather than producing a successful empty report.

Optional `remark` follows the existing task contract: trim surrounding whitespace, allow at most 2,000 Unicode code points, include it in idempotency comparisons, and return it only in authorized `TaskDetail`. It never enters the engine, audit metadata, or reports.
...
Only `aig migrate` may apply database DDL. A fresh or upgraded PostgreSQL database schema reaches v10; runtime startup only validates it. Migration safely handles an empty legacy table and never relies on runtime AutoMigrate. Version 8 adds `idx_platform_tasks_updated_at` and `idx_platform_tasks_owner_updated_at` for stable recent-task ordering (`updated_at DESC, id DESC`) in global and owner scopes. Version 9 idempotently backfills ready attachments already referenced by historical tasks to attached while leaving unbound ready attachments eligible for cleanup. Existing version 10 adds `platform_tasks.remark` and `platform_tasks.target_count`; Agent workflow scanning reuses this baseline and adds no migration.
...
- `GET /api/v1/platform/reports/{reportID}` returns a safe detail whose `render` derives from the immutable RenderModel stored with the completed task. For MCP reports, online detail and exported PDF replace technical findings at the presentation boundary with server-generated fixed category/severity summaries; real targets, repository identifiers, and scanner free text are not returned.

The immutable `report-render-v2` RenderModel freezes the risk mapping version, generated/completed times, task metadata, product/color/watermark, risk score and score explanation, risk distribution, exactly 30 fixed UTC day buckets in `risk_trend`, Top risks, technical findings with evidence/impact/remediation, recommendations, coverage, and conclusion. For a trusted AI infrastructure task, it may contain `port_scan_mode` and `port_spec` together, but only as `fixed_ai` + `11434,1337,7000-9000,18789` or `full_tcp` + `1-65535`; absent, isolated, unknown, or mismatched values must omit both fields. Technical findings are explicitly mapped from the four trusted engine schemas, redacted, severity-ordered, and capped at 50; prompts, conversations, attachments, screenshots, credentials, URL queries, and user paths are never copied into the render model. Before presentation, MCP online detail and paginated PDF retries additionally project technical findings into fixed generic summaries, omitting real targets, repository identifiers, and scanner free text. The list and detail contracts are deliberately separate: raw engine results and Logo bytes are never exposed, nor are stored render payloads, owner IDs, file paths, or mutable brand records. List pagination accepts `page=1..1000` and `page_size=1..100` (default 20).

Skills use the same validated `score`/`results`/`level` result conversion as MCP, while snapshot, online-detail, and report-list `task_type` remains `skills_scan`. Existing MCP reports and historical task-type aliases retain their original values and are never reclassified.
