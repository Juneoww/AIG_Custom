...
**Breaking change:** `POST /api/v1/platform/tasks` no longer returns the former internal task `View`. A successful `202` now returns `TaskDetail` with `id`, display-safe `owner`, canonical `task_type`, `status`, `created_at`, `updated_at`, bounded `input_summary`, and optional `remark`. This prevents stored request and engine internals from crossing the browser boundary; lists retain their six safe fields and omit remarks.
