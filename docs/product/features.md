...
| REPORT-02 | 风险报告、趋势与 PDF | 四类真实扫描结果映射与技术发现脱敏 | 将 AI 基础设施、MCP、Agent 工作流和模型红队结果统一为风险、影响与建议，不暴露原始密钥、私有路径、Prompt 原文或完整会话内容 | 普通用户、安全审计员、系统管理员 | Web/API/报告服务 | 已实现 | 原生能力 | E-REPORT-02 |
...
| REPORT-05 | 风险报告、趋势与 PDF | 同一不可变展示快照生成中文多页 PDF | 在线详情与导出使用同一快照，PDF 嵌入中文字体并在每页应用水印；重复导出不重新读取引擎结果或当前品牌 | 普通用户、安全审计员、系统管理员 | Web/API/PDF | 已实现 | 原生能力 | E-REPORT-05 |
...
| AUDIT-01 | 审计、恢复与安全防护 | pending、completion 与 outbox 持久化变更审计 | 关键治理变更先记录意图，再持久保存完成结果；投递失败可对账恢复，避免业务已成功却因审计短暂故障被误报失败 | 安全审计员、系统管理员 | Web/管理 API/审计服务 | 已实现 | 原生能力 | E-AUDIT-01 |
...
| AUDIT-03 | 审计、恢复与安全防护 | 有界任务完成、报告与并发恢复 | 平台从持久结果恢复任务完成和缺失报告，每轮处理数量及单项时间均受限；多个处理者竞争时只产生一个有效结果，超时处理权可安全接续 | 系统管理员、运维交付人员 | 后台恢复/管理 API | 已实现 | 原生能力 | E-AUDIT-03 |
...
| OPS-02 | 数据迁移、部署与运维 | PostgreSQL Compose 基线、Agent 内部凭据与附件限制 | 当前 Compose 交付 PostgreSQL 健康检查和一次性迁移容器；平台与 Agent 运行时共享独立内部 Token，缺失时失败关闭，浏览器仍使用会话与 CSRF；附件总量和分片上限可配置，平台与 Agent 完整 Compose 编排仍待补齐 | 系统管理员、运维交付人员 | 部署配置/CLI/Agent | 部分实现 | 原生能力 | E-OPS-02 |
| OPS-03 | 数据迁移、部署与运维 | 多架构镜像、版本发布工作流与第三方许可随包 | 自动构建服务器和 Agent 多架构镜像及版本归档，并随镜像和发布包携带内嵌 PDF 字体许可及完整第三方声明 | 运维交付人员 | 镜像/发布工作流 | 已实现 | 原生能力 | E-OPS-03 |
