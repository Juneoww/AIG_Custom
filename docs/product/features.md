...
| 安全审计员 | 全局只读查看任务、脱敏报告、趋势和审计记录，可在授权范围内导出不可变报告 | 禁止读取或下载任何原始附件 |
| 系统管理员 | 治理用户与角色、模型、规则、品牌、任务恢复、报告补建和系统数据，可全局查看任务、报告与审计 | 跨所有者原始附件只允许用于治理场景，并在打开存储前形成持久、脱敏授权审计 |
...
| FILE-03 | 附件管理 | 私有下载、任务绑定与安全回收 | 普通用户下载仅限本人附件；创建任务时只能原子绑定同一所有者已完成附件；未绑定 uploading/ready 附件经 deleting 墓碑清理，绑定后不可由中止接口删除 | 普通用户、系统管理员 | Web/API | 已实现 | 原生能力 | E-FILE-03 |
...
| FILE-06 | 附件管理 | 跨所有者原始附件治理边界 | 安全审计员不能读取或下载原始附件；管理员跨所有者打开存储前必须持久化脱敏 `attachment.download_authorized` 授权事件 | 安全审计员、系统管理员 | Web/API | 已实现 | 原生能力 | E-FILE-06 |
...
| AUDIT-04 | 审计、恢复与安全防护 | 服务端授权、审计员只读与最小权限运行 | 权限判断以服务端会话和角色为准，审计员对审计、任务和脱敏报告只有全局只读权限且不能读取原始附件；运行账号无需结构变更权限，启动仅校验所需数据库对象 | 安全审计员、系统管理员、运维交付人员 | Web/API/服务启动 | 已实现 | 原生能力 | E-AUDIT-04 |
...
5. **审计员全局只读审阅**：安全审计员可全局查看任务、脱敏报告、趋势和审计记录，并按授权导出报告，但不能查看或下载原始附件。
...
- **E-FILE-03**：运行入口——平台附件下载、任务创建附件绑定与附件回收；验证来源——`internal/platform/tasks/handler_test.go` 的普通用户跨所有者下载拒绝测试，`internal/platform/tasks/service_test.go` 的同所有者 ready→attached 绑定、绑定后拒绝中止、deleting 墓碑重试和 TTL 有界回收测试，以及 `pkg/database/migrate_test.go` 的 v9 历史绑定回填测试。
...
- **E-FILE-06**：运行入口——平台附件读取与下载入口；验证来源——`internal/platform/tasks/service_test.go` 的审计员拒绝、普通用户 404 隐藏和管理员跨 owner 授权审计持久化/失败关闭测试。
