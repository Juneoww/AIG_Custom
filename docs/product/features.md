...
| FILE-05 | 附件管理 | 旧公开图片与浏览器上传路由退役 | 旧公开图片、整文件上传、分片上传和合并入口不再作为浏览器能力，受保护检查后统一返回 410 | 普通用户、运维交付人员 | Web/API | 已实现 | 退役边界 | E-FILE-05 |
...
| KB-02 | 规则与知识库 | 中英文漏洞库受控读取与编辑 | 统一维护中文和英文漏洞知识，使扫描与治理人员能够按语言查看内容，并由系统管理员受控更新 | 普通用户、安全审计员、系统管理员 | Web/API/规则数据 | 已实现 | 原生能力 | E-KB-02 |
...
| KB-04 | 规则与知识库 | MCP 数据、Agent 配置与越狱规则受控管理 | 支持受控读取 MCP 数据、Agent 配置和越狱评测规则，并由系统管理员通过已有知识库写入口维护对应内容，不据此扩展未注册扫描类型 | 普通用户、安全审计员、系统管理员 | Web/API/规则数据 | 已实现 | 原生能力 | E-KB-04 |
| KB-05 | 规则与知识库 | 知识库与规则数据版本查询、受控更新和同步 | 安全审计员与系统管理员可查询数据版本及同步状态，系统管理员可触发受审计的规则数据更新，减少离线规则长期陈旧 | 安全审计员、系统管理员 | Web/API/规则数据 | 已实现 | 原生能力 | E-KB-05 |
