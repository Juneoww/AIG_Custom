...
任务持久化参数与 Agent 分配参数分离。MCP 新分配只允许空 `Content`、固定 `zh_CN`
及经过白名单校验的扫描参数；端点、认证字段和附件内部路径不能借助延迟运行时签发器
进入引擎 Session。服务来源的分配仅增加内部网关地址、任务能力令牌和具体传输协议；
仓库来源只增加不透明归档引用。

签发器在选定 Agent、准备好新的分配后才运行，并继承调用者的取消信号和有限的准备超时。
重复提交已接受的任务只做状态核对，不轮换该 Agent 正在使用的令牌。无可用 Agent、模型
解析失败或分配失败时清除直接运行时材料；重试必须重新从加密绑定签发，不能从 Session
或缓存恢复旧令牌。尚未写出 Agent 消息的失败只释放当前待发送的分配，不重开已运行任务。

网关的域内 HTTP 适配器只允许内部 Agent 中间件保护的路由，每次请求还校验任务能力、
任务状态和固定连接版本；长连接受能力有效期限制，并持续复核任务及配置是否仍可用。
生产代理构造器不提供任意 HTTP client 替换口。SSE 上游消息地址只在网关内存中保留，
向 Agent 返回任务绑定的不透明会话路径，不回传真实主机或服务端会话查询参数。
Streamable HTTP 的上游会话标识也映射为不透明标识，不允许 Agent 直接提交上游会话。
会话映射同时校验任务与能力轮换版本；结束、过期、轮换后删除，并限制单任务及总会话数。

Agent 事件先按加密任务绑定脱敏，再进入事件存储、日志和浏览器 SSE；JSON 字段名和
字段值都属于需要清洗的数据。脱敏器不可用时只保留固定安全提示；如果无法得到安全的
最终结果，则记录任务失败，不能把无法恢复的空结果标为成功。

生产入口使用 MCP 专属浏览器 API、连接配置和受控执行链路。部署开放前仍须完成密钥、
允许集、Agent 内部认证及实际连接的集成验收；不能因域内单元测试通过就放开旧的直连执行路径。

### 模型可选与报告覆盖范围

新建任务可以不选择模型，省略 `model_id` 时不读取默认模型，也不使用环境中的模型凭据。
仓库基础检查只读取受控归档内的源码或配置，以固定规则查找动态执行、Shell 调用和危险反序列化
线索，不执行目标代码。服务基础检查仅执行 MCP 初始化和 `tools/list`，检查工具输入声明、
破坏性操作声明和明显描述指令污染；不会调用远程工具。没有可分析源码、连接失败或结果超过
边界时任务失败，不把未分析的输入计为安全。

选择受治理模型后增加模型辅助分析；仓库分析工具仍限制在归档根目录内，只读且不能调用 Shell。
服务的模型辅助分析可通过任务网关调用已授权服务的工具，因此新建服务任务必须确认测试授权。
两种模式都不允许通过模型参数覆盖 MCP 网关、传输协议或连接凭据。

基础报告显示“基础检查（未使用模型）”，规则命中只作为待复核线索；0–100 分为基础规则参考分。
未发现线索或取得高分不等于完成全面安全评估、证明无漏洞或取得安全认证。报告、共享视图和
导出复用相同的覆盖范围、评分说明与结论。
